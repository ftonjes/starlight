...
def analyze(output, collection=None):
    ...
    """
        ...
        Cisco - 'show startup-config' command analysis

    :param output: output from 'show startup-config' on a Cisco device (type: str)
    :param collection: collection to which analyzed information is added to
    :return: collection with included analysis from 'show startup-config' command
    """

    # Version: 1.0 - Initial version
    # Description: Analyze 'show startup-config' output from a Cisco device
    # Regex: '^show startup-config$'
    ...
    if not isinstance(output, str):
        return False, "Output from 'show startup-config' command should be in string format."
