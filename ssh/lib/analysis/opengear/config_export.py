...
def analyze(output, collection=None):
    ...
    """
        ...
        Netscout - 'config export' command analysis

    :param output: output from 'config export' on an Opengear device (type: str)
    :param collection: collection to which analyzed information is added to
    :return: collection with included analysis from 'config export' command
    """

    # Version: 1.0 - Initial version
    # Description: Analyze 'config export' output from an Opengear device
    # Regex: '^config export$'
    ...
    if not isinstance(output, str):
        return False, "Output from 'config export' command should be in string format."
